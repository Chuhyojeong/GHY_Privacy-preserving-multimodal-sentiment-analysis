...
class LSTM_Model():
    ...
    def self_attention(self, inputs_a, inputs_v, inputs_t, name):
        ...
        kernel_init1 = tf.compat.v1.glorot_uniform_initializer(seed=self.seed, dtype=tf.float32)
    ...
    def self_attention_2(self, inputs, name):
        ...
        # dense_attention_2 = Dense(attention_size, activation=None,kernel_initializer=init1,kernel_regularizer=slim.l2_regularizer(0.001))
    ...
    def _build_model_op(self):
        ...
        init = tf.compat.v1.glorot_uniform_initializer(seed=self.seed, dtype=tf.float32)
        if self.unimodal:
            self.inter1 = Dense(100, activation=tf.nn.tanh,
                                kernel_initializer=init, kernel_regularizer=slim.l2_regularizer(0.001))(
                self.inter)
        else:
