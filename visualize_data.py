...
def unimodal(mode, data, classes):
    classes = int(classes)
    print(('starting unimodal ', mode, data))
    ...
    if data == 'cifar10':
        ...
        train_label = tf.keras.utils.to_categorical(train_label, classes)
        test_label = tf.keras.utils.to_categorical(test_label, classes)

        # Prepare masks and sequence lengths
        train_mask = np.ones((train_data.shape[0], train_data.shape[1]), dtype='float32')
        test_mask = np.ones((test_data.shape[0], test_data.shape[1]), dtype='float32')
        seqlen_train = np.ones(train_data.shape[0], dtype='int32') * train_data.shape[1]
        seqlen_test = np.ones(test_data.shape[0], dtype='int32') * test_data.shape[1]

    # with open('./mosei/text_glove_average.pickle', 'rb') as handle:
    elif data == 'mosei' or data == 'mosi':
    ...
    with tf.device('/device:GPU:%d' % gpu_device):
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                for epoch in range(epochs):
                    epoch += 1
                    minsize = min(len(train_data), len(train_mask), len(seqlen_train), len(train_label))
                    batches = batch_iter(list(
                        zip(train_data[0:minsize], train_mask[0:minsize], seqlen_train[0:minsize], train_label[0:minsize])),
                        batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default='cifar10')  # Set dataset to CIFAR-10
    parser.add_argument("--classes", type=int, default=10)  # Set number of classes for CIFAR-10
    ...
    assert args.data in ['cifar10', 'mosei', 'mosi', 'iemocap']  # Add 'cifar10' to the list
